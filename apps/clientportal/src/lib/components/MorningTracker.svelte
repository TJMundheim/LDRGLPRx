...
<div class="card" style:border-color="{wc.ac}55">
  ...
  {#if steps.length > 0}
    <div class="steps-list" style="margin-bottom:12px">
      <div style="font-size:10px;font-weight:700;letter-spacing:.07em;color:var(--mc-muted);margin-bottom:6px;text-transform:uppercase">
        Active steps{week4Note}
      </div>
      <ol style="margin:0;padding-left:18px;display:flex;flex-direction:column;gap:5px">
        {#each steps as step}
          <li style="font-size:12px;color:var(--mc-ink);line-height:1.4">
            <strong>{step.n}</strong> — <span style="color:var(--mc-muted)">{step.p}</span>
            {#if step.u}
  ...
  <div style="font-size:11px;color:var(--mc-muted);margin-bottom:8px">Tap each day you completed all elements</div>
  <div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:12px">
    {#each days as d, i}
      {@const key = `w${week}d${i+1}`}
      {@const done = !!log.morn[key]}
      <button class="day-btn"
        style:background={done ? wc.ac : 'var(--mc-panel)'}
        style:color={done ? 'var(--mc-on-gold)' : 'var(--mc-muted)'}
      ...
      <div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:4px">
        {#each days as d, i}
          ...
          <button class="cold-btn"
            style:background={done ? 'var(--mc-info)' : 'var(--mc-panel)'}
            style:color={done ? 'var(--mc-on-gold)' : 'var(--mc-muted)'}
