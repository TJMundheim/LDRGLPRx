<script lang="ts">
  ...
  function bandInfo(score: number): { title: string; body: string; color: string } {
    ...
    if (score <= 5) return {
      ...
      body: 'Start with Biome NS Ultra to support your gut-immune barrier and consider a 30-day elimination protocol to identify your specific triggers.',
      color: '#D4920A',
    };
    return {
      title: 'Significant allergy/sensitivity load.',
      body: 'Strong indication for an elimination diet combined with Biome NS Ultra. Flag for a telemedicine consult to discuss IgG/IgE testing options.',
      color: '#E05C2A',
    };
  }
</script>
