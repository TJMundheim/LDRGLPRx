...
/** Step 1: request a one-time code sent to the user's email. Returns the session string. */
export async function requestEmailCode(email: string): Promise<string> {
  ...
  if (!result.Session) throw new Error('No session returned from InitiateAuth');
