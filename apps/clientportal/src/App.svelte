  ...
  function allergyAssessmentAction(action: string, qIndex?: number, value?: boolean): void {
    ...
    if (action === 'submit') {
      ...
      if (score <= 2) {
        ...
        body = 'Start with BiomeAxisForge to support gut-immune barrier; consider a 30-day elimination protocol.';
        scoreColor = '#D4920A';
      } else {
        title = 'Significant allergy/sensitivity load.';
        body = 'Strong indication for elimination diet + BiomeAxisForge; flag for telemedicine consult to discuss IgG/IgE testing.';
        scoreColor = '#E05C2A';
      }
