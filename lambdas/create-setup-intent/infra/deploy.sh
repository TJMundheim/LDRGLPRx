#!/usr/bin/env bash
# Deploy create-setup-intent Lambda — idempotent
set -euo pipefail

FUNCTION_NAME="my4mlife-create-setup-intent"
ROLE_NAME="my4mlife-create-setup-intent-role"
REGION="us-east-2"
...
aws lambda remove-permission --function-name "$FUNCTION_NAME" --region "$REGION" \
  --statement-id apigw-create-setup-intent >/dev/null 2>&1 || true
aws lambda add-permission --function-name "$FUNCTION_NAME" --region "$REGION" \
  --statement-id apigw-create-setup-intent \
...
for r in "POST /api/create-setup-intent" "OPTIONS /api/create-setup-intent"; do
...
echo "==> Done: POST+OPTIONS /api/create-setup-intent → $FUNCTION_NAME"
