...
FUNCTION_NAME="my4mlife-log-parse"
ROLE_NAME="my4mlife-log-parse-role"
REGION="us-east-2"
...
# JSON env file (shorthand syntax mangles some values).
ENV_FILE="$(mktemp)"
cat > "$ENV_FILE" <<JSON
{"Variables":{"BEDROCK_MODEL":"$BEDROCK_MODEL"}}
JSON
...
# ── 4. HTTP API route ─────────────────────────────────────────────────────────
log "Wiring HTTP API route POST /api/log-parse..."
ROUTE_KEY="POST /api/log-parse"
...
if [[ "$EXISTING_ROUTE" != "None" && -n "$EXISTING_ROUTE" ]]; then
  ...
  $AWS lambda add-permission \
    --function-name "$FUNCTION_NAME" \
    --statement-id "apigateway-log-parse" \
    --action "lambda:InvokeFunction" \
    --principal "apigateway.amazonaws.com" \
    --source-arn "arn:aws:execute-api:$REGION:$AWS_ACCOUNT_ID:$API_ID/*/*/api/log-parse" \
    2>/dev/null || true
...
log "Done. Endpoint: https://$API_ID.execute-api.$REGION.amazonaws.com/api/log-parse"
