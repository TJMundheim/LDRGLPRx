...
# JSON env file (shorthand syntax mangles URLs containing "://").
ENV_FILE="$(mktemp)"
cat > "$ENV_FILE" <<JSON
{"Variables":{"CONTACT_TABLE":"$CONTACT_TABLE","CONVERSATIONS_TABLE":"$CONVERSATIONS_TABLE","CONCIERGE_FROM":"$CONCIERGE_FROM","BEDROCK_MODEL":"$BEDROCK_MODEL","CONCIERGE_MODE":"$CONCIERGE_MODE","NOTIFY_TO":"$NOTIFY_TO","EMAIL_SENDER_FN":"$EMAIL_SENDER_FN","APPROVE_URL":"$APPROVE_URL","APPROVE_SECRET":"$APPROVE_SECRET"}}
JSON
