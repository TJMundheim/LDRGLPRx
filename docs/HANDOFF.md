...
- **App shop-stack CTA (2026-09-24):** `src/lib/renderer.ts` — new `shopStackButton()` links to `https://my4mlife.com/stack?utm_source=app` (new tab, PostHog `app_shop_stack` if `window.posthog` present); rendered at the top of `renderSupplementsPanel()` (Week 3 + Regen tabs) and in Week 1's Mitigate section. Tests extended in `renderer.sex.test.ts`. Built + deployed via `apps/clientportal/deploy.sh`.
...
- **TJ directive "back to basics":** proper intake → proper coordinator visit → proper direction, delivered automatically. First real user: **Brian Schumacher, call Wed 2026-09-16** — send him to https://www.my4mlife.com/consult (NOT the old 6-field form).
- **SHIPPED + DEPLOYED (9b1fd4e9 · 149c9c23 · f7732d9b):** `/consult` = 5-step Care Coordinator Intake, no card (identity · why now/curiosity/refusals/budget · health history · lifestyle · review+consent) → contact-form (lead email + confirmation) + patient-record-intake (category `care-coordinator`). patient-record-intake now invokes **`my4mlife-coordinator-brief`** → Bedrock Haiku → `brief#<enc>` on PatientRecords + email to drtj@ "[Pre-call brief] <name> — <top lane> — call <bestTime>" (pulls MindSpan intakeAnswers/auditTop3 from Contact when present). **`my4mlife-plan-of-action`**: draft (Bedrock, coordinator notes + brief) → `plan#<enc>`; send = upsert 'sending' → email patient from info@ (link allowlist my4mlife.com only; DISCLAIMER always appended) → 'sent' + audit + "[Plan sent]" copy to drtj@. AppSync: BriefAdmin/PlanAdmin, `briefs`/`plans` on PatientRecordAdmin, mutations generateCoordinatorBriefAdmin / draftPlanOfActionAdmin / sendPlanOfActionAdmin (Admins). Admin app → Patients → encounter: **Pre-call brief** panel (Generate/Regenerate) + **Plan of action** panel (notes → Draft with AI → edit → Send to patient). Plan: docs/plan/care-coordinator-intake-brief-plan-2026-09-10.md.
- **E2E PROVEN 2026-09-10 19:59Z** on test record drtj+intaketest@ (contactId 3f4823b2-…, enc 92393288-…): brief stored (lanes Gut-Brain Rx, GLP-1, OTC foundation), plan drafted, plan sent (state sent, audit rows). Three emails fired. Test rows left in PatientRecords (harmless; delete if desired).
- **BRIAN CALL-DAY RUNBOOK:** (1) he completes /consult → TJ gets "[Pre-call brief]" email within ~1 min; (2) before the call read it (or app.my4mlife.com → Admin → Patients → his encounter → Pre-call brief); (3) after the call: Plan of action panel → type what you told him → Draft with AI → edit → Send to patient → he gets the email, TJ gets the copy.
- **Fixes found by review/E2E (all applied):** AWSJSON double-encoding of brief; listPatientRecordsAdmin missing briefs/plans (would 500); demographics.email now stamped on every intake (rx funnels too); markPlanSent set a partition key (DynamoDB rejects) + store-before-send reorder; Bedrock IAM parity (aws-marketplace:ViewSubscriptions/Subscribe) — first invoke after deploy can fail ~5 min on propagation.
- **FLAGS for TJ:** (a) success screen says "Dr. TJ will call you <bestTime>" — personal-availability promise; soften to "a care coordinator" if desired. (b) Existing PatientRecords rows predate the demographics.email fix → plan send would throw "no patient email" for them; one-time backfill if needed. (c) **`lambdas/coach-proxy` still imports @anthropic-ai/sdk directly — violates the Bedrock-only rule (pre-existing); migrate.** (d) 15 lambda files exceed the 100-line rule (pre-existing); intake handler is 112.
...
- **DEPLOYED 2026-09-09 (all 3 scripts run by TJ; APPROVE_URL env fixed by hand + deploy.sh switched to JSON env file). End-to-end test via synthetic S3 event: bug/0.95/escalate → draft stored → notification sent to drtj@ → TJ clicked Approve → reply delivered to outside mailbox (CONFIRMED 2026-09-09). Fix applied en route: NONE-auth function URLs need BOTH lambda:InvokeFunctionUrl and lambda:InvokeFunction (invoked-via-function-url) since Oct 2025 — now in deploy.sh. REMAINING: Google Admin routing rule (step 4, click-by-click in docs/ops/google-routing-concierge.md) — until then no real support@ mail reaches the concierge.**
- **Original deploy order (for redeploys):** `./lambdas/concierge-approve/infra/deploy.sh` → `./lambdas/inbound-handler/infra/deploy.sh` → `./lambdas/inbound-handler/infra/deploy-ses-receiving.sh` → Google Admin routing rule per `docs/ops/google-routing-concierge.md` → test by emailing support@ from an outside account.
...
- **2026-09-08 SHIPPED (all committed to main):** title LOCKED "The Uninsured Decade"; honesty spine approved (no break-even claim, insurance = ruin avoidance); gatekeeper materials GENDER-NEUTRAL, book keeps Dr. TJ's male seat as voice. **Channel 0 added = financial-services dinner seminars (TJ's existing regen-med host relationships; host pays room+marketing; 8-min guest slot + books) — run first.** Deliverables: docs/launch/uninsured-decade/ (speaker sheet md+html, advisor leave-behind md+html, 5 pitch emails + phone script, 8-min dinner script w/ annuity-host variant) · docs/book-uninsured-decade/draft/_MASTER.md (front matter + Part I Ch1–5, ~8k words) · **website /go/uninsured-decade calculator BUILT + browser-verified, NOT YET DEPLOYED** (deploy.sh blocked by permission classifier — TJ runs `bash website/deploy.sh`). Part I flags for TJ: Ch5 "Second Patient" placed at end of Part I (outline had it opening Part II); Ch3 names Genworth/John Hancock with rate-hike %s from public filings (keep named or anonymize?); Ch3 "three questions" insurance sidebar needs a decision. Unverified $ in Part I: adult day ~$9,900/yr. Next: Part II–IV draft, QR handout, TJ to supply dinner-host names.
- **2026-09-08 (later):** lifetime-risk basis corrected to Nature Medicine 2025 (42% single / 35% men / 48% women / ~66% couple); Lancet 45% = labeled population ceiling only; 30% individual default. "The Dividend" (healthspan payout) locked as payout #2 — Part II opens with it. **Tagline standing rule (memory feedback_identity_dignity_tagline):** "Don't lose your identity and your dignity while you still have a choice." on every My4MLife surface — swept into footer, homepage, assessment, clientportal sign-in, new book title page + Ch6, speaker sheets; CYS = bridge point only (last chapter; NOT app home). **Part II drafted + committed** (Ch6–9, ~7.2k words; total draft ~17k). Flags for TJ: Ch8 trainer line ($280/mo = 40% of basket — keep or drop changes every premium figure); Ch8 LTC-premium comparison needs a compliance read; Ch9 physical-activity meta-analysis RRs need named primary cites before print. **LEAD CONCEPTS = POINT OF THE SPEAR:** Chris Wear (owner, TJ's friend), seminar direct-mail ~100K/wk nationwide to annuity/trust/estate hosts; TJ meets him wk of 09-08; brief at docs/launch/uninsured-decade/lead-concepts-meeting-brief.md; memory project_lead_concepts_spear. Website deploy still pending TJ (`bash website/deploy.sh`) — calculator + footer/homepage/assessment tagline changes are all undeployed.
- **DRAFT v1 COMPLETE (2026-09-08, commit after 1849ed01):** docs/book-uninsured-decade/draft/_MASTER.md — 15 chapters + Signing + The Assessment + About the Author + Sources, ~30.7k words (≈120pp at 6×9 vs the 80pp target — TJ to choose: keep long or cut ~10k). Ch9 citations corrected (Iso-Markku 2022 / Feter 2026 / Ngandu 2015 / Qi 2024 / Baker 2025 — docs/…/source/citations-part2-2026-09-08.md). Part III citation pass running → source/citations-part3-2026-09-08.md (6 UNSOURCED tags: gut-brain neurochem, myokines/BDNF, sarcopenia rate, ED sentinel, alcohol/REM, EPA indoor air). Credential rule 2026-09-08: "Dr. TJ" only, no ", DC" on educator surfaces; calculator carries no author name. **TJ review calls outstanding (9):** P1 Ch3 named carriers · Ch3 insurance sidebar · Ch5 placement; P2 Ch8 trainer line · Ch8 LTC-premium comparison; P3 Ch11 "fitter lives longer with it" · Ch10–12 Rx category naming · Ch13 Cindy's near-conversations; P4 Ch14 "already diagnosed" paragraph · Ch15 labs/hormone-panel naming · Ch15 "sleep first" promise. Next: Part III cites → length decision → build pipeline (reuse docs/book/build_ebook_docx.py + render) → cover.
- **2026-09-08 (evening):** calculator inputs fixed — every input drives the ledger (age→onset-year inflation 4%/yr; spend→new money vs $81K basket; hours→hours-to-add; spouse→onset + couple odds); verdict sentence reads the result (a couple in their 60s now clears break-even on dementia alone in most states — Ch9 gets one sentence on this in the length pass). Vagal 90%→~80% corrected sitewide + concierge prompt (flagship book at v21). **Homepage + navbar: Gut-Brain Rx now FIRST, GLP-1 second (TJ after phone review).** **CYS landing page decided:** own page on my4mlife.com (LP template; book button → Amazon; app button only once App Store-listed) — NOT built yet because the CYS book has no KDP/ASIN listing (no-coming-soon rule); build the day the book is live. **WEBSITE DEPLOYED 2026-09-08 (TJ ran deploy.sh; Fable verified live):** calculator /go/uninsured-decade, tagline (footer/homepage/assessment), vagal ~80% fix, Gut-Brain-first reorder, no author name on calculator.
- **DRAFT v2 (2026-09-08 night): 80-page cut DONE** — 15,200 words, 13 chapters (Ch2+3 merged "What Nobody Pays For"; Ch7+8 merged "Ten Hours and $675"), renders to **82pp** (The-Uninsured-Decade-draft-v2.pdf, also on TJ Desktop). Long v1 archived at draft/_MASTER-v1-long-archive.md. Render: part kickers `{.part}` share the chapter page; Sources compact. **NEW LOCK (4) endorse-the-professional-partner** (memory feedback_endorse_the_professional_partner): applied to book (Ch1/2/3/6/7/10/12/13 + Assessment page), dinner script, leave-behind, pitch emails, speaker sheet, plans, Lead Concepts brief, calculator copy. The cut also resolved 3 TJ calls (insurance sidebar → 3 sentences; "fitter lives longer with it" removed; already-diagnosed → 1 sentence). **Remaining TJ calls (6):** Ch2 named carriers (Genworth/John Hancock) · Ch4 placement · Ch6 trainer line · Ch6/7 LTC comparison (now non-ranking) · Ch11 Cindy's near-conversations · Ch13 "sleep first" promise. **FLAG:** subtitle "The one catastrophic risk you cannot buy a policy against" is more absolute than the interior now is — consider "…the one risk no policy can lower" before cover design. Calculator copy changed after the deploy → one more `bash website/deploy.sh` needed (bundle with next change). Next: cover + QR codes + KDP prep once TJ signs off; CYS landing page once CYS book has an ASIN.
- TJ is working the punch list (delivered 2026-09-07, 7 sections; item 8 removed) on 2026-09-08.
...
- CYS toolchain RESOLVED (verified 2026-09-07): CountYourselfSkinny.xcodeproj is committed (f406b06f) and builds clean with Xcode 26.6 on the iPhone 17 sim. xcodegen/Homebrew still absent but NOT needed (only for regenerating from project.yml). No TJ install action required. Book v2 82pp unchanged.
...
- **INBOUND EMAIL — support@ CONFIRMED WORKING 2026-09-03** (a Protégé's email to support@my4mlife.com reached drtj@; MX → Google Workspace, alias/catch-all now routes it). **info@ (welcome-email FROM / reply-to) CONFIRMED WORKING 2026-09-05** (TJ test delivered to drtj@). Both public addresses now route to TJ; no open email-routing items. Outbound from info@ needs nothing (SES domain identity verified). SES inbound-handler can't be live (MX points to Google).
...
## 2026-09-22 (night) — MIDLIFE / BOTH-SEXES REFRAME: ALL SEVEN ITEMS SHIPPED
Direction locked (docs/plan/all-genders-midlife-reframe-2026-09-22.md). Tonight, all live/pushed:
1. Sex captured (assessment one-tap + consult prefill) → UserProfile.sex. 2. /women + /men doors + homepage "Who are you?" row + midlife sub-line.
3. Photos: TJ generates in **Artlist** (Google Imagen; NOT Midjourney — docs corrected). Placed: w3-02 women hero, w3-08 women group, w3-12m men group; men hero interim = regen-ridge; homepage has no people photo in coordinator mode. Wave 3 remaining: w3-01, w3-11 + 9 others (docs/design/wave3-artlist-prompts.md). Downloads land in ~/Downloads under Artlist names; Fable matches by eye.
4. Site copy sweep (6 files; spine sentence on Mind/Motivate/assessment). 5. App branches on sex (who() in src/lib/sex.ts) + Week 3 hormones section (ED + perimenopause canaries). 6. Logbook v9 155pp on S3 (both keys). 7. Book v23 315pp: question-nine fix, Elena week, James 52, midlife spine ×2; covers re-spun (B&W 0.709in, color 0.739in); Desktop KDP-UPLOAD v23 files; S3 refreshed. NOT uploaded to KDP — TJ reviews v23 tomorrow. Hardcover wrap still ~259pp, needs re-spin before hardcover upload.
Open: pre-2026-09-22 profiles have no sex (paired copy shown); stale docs/cohort-workbook/draft/01-09 chapter files; /about "Patriarch" photo awaits Wave 3; nav has 7 links.
