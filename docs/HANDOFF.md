...
- Women's HRT product line still needs telemed partner prescribing. **CYS APP RUNS ON SIMULATOR (2026-09-02):** Xcode 26.6 + iOS 26.5 runtime installed; iPhone 17 Pro sim UDID DFE0EF33-…; build via MCP build tool OK, launch via `xcrun simctl bootstatus -b` + `simctl install/launch` (MCP launch reports Shutdown — use simctl). Home screen verified. Next: TJ Apple Developer account → StoreKit products + offer code; then v1.5 AirPods prototype.
