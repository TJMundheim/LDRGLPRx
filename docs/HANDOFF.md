...
- **DEPLOYED 2026-09-09 (all 3 scripts run by TJ; APPROVE_URL env fixed by hand + deploy.sh switched to JSON env file). End-to-end test via synthetic S3 event: bug/0.95/escalate → draft stored → notification sent to drtj@. REMAINING: Google Admin routing rule (step 4) — until then no real support@ mail reaches the concierge.**
