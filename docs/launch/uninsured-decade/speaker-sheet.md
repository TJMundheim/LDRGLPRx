# Speaker Sheet — "The Uninsured Decade"

_Drafted 2026-09-08. Companion to docs/launch/speaking/booking-email-and-speaker-sheet.md — mirrors format and voice. Gender-neutral throughout per docs/book-uninsured-decade/README.md locked rule. Content for a one-page PDF; rendered as speaker-sheet.html._

---

**DR. TJ** — Health-span educator · 30+ years in clinical practice · Fort Worth, TX
Author of *The Uninsured Decade* and *Begin with the End in Mind* · Founder of My4MLife

## Headline

### The Uninsured Decade
*The one retirement risk your clients can't buy insurance for — and what the numbers actually say about it.*

Medicare pays nothing toward custodial care. The private long-term-care insurance market has largely collapsed — fewer than a dozen carriers still sell new standalone policies. And the average dementia care journey costs a family $405,262, about 70% of it out of their own pocket and their own hours. This talk lays out the bill in plain numbers, then reframes the response: you can't insure this risk anymore, but you can underwrite it.

*Don't lose your identity and your dignity while you still have a choice.*

## Three takeaways

1. **The bill is bigger and more certain than most retirement plans account for** — a ten-year staged cost model, Medicare's exact coverage gap, and the Medicaid spend-down math any household can run for itself.
2. **Insurance stopped being the backstop.** Rate hikes, tightened underwriting, and carrier exits mean the old "just buy a policy" answer no longer holds for most families walking into this conversation today.
3. **There is still a lever — modifiable risk factors, honestly framed.** Expected-value thinking, not a guarantee: nothing here prevents dementia, but the evidence supports meaningfully shifting the odds.

## Formats

- **40-minute masterclass + Q&A** — full talk: the scenario, the $0 Medicare line, the spend-down math, the reframe to underwriting, the modifiable-risk-factor case, close and QR. Built for estate-planning councils, CPA/CE sessions, and CEO peer groups.
- **8-minute guest slot** — for financial-services client dinners (Channel 0): the bill in five numbers, the underwriting reframe, one QR slide. Verbatim script at docs/launch/uninsured-decade/dinner-guest-slot.md. Fits inside another host's agenda without competing with their close.

Both formats: no AV required (slides optional), fits any room from a private dining room to a chapter meeting.

## What's NOT sold from the stage

Nothing. No product, no supplement, no company name, no consult booking pitched from the podium. The only ask, in both formats, is a free seven-minute risk assessment reachable by QR code. Books are left on the table or the seat, not sold. This is a standing rule across every venue (see docs/plan/speaking-circuit-traffic-strategy.md and docs/plan/uninsured-decade-channel-plan-2026-09-07.md).

## AV needs

None required. A projector/screen for the optional slide deck and a lavalier or handheld mic for rooms over ~40 seats. No internet connection needed — the QR code is a static image.

## Contact

Dr. TJ · drtj@essentialmanage.com · [phone] · my4mlife.com

---

_Health and financial-risk education, not medical, financial, or legal advice. Attendees are encouraged to work with their own physician and advisor. Dr. TJ speaks as a health-span educator and does not provide medical care through this platform._
